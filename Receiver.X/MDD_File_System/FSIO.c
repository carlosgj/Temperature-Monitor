...
uint8_t Alias_LFN_Object(FILEOBJ fo);
...
// Write functions
    ...
    CETYPE CreateFileEntry(FILEOBJ fo, uint16_t *fHandle, uint8_t mode, uint8_t createFirstCluster);
#endif
...
CETYPE FILEfind( FILEOBJ foDest, FILEOBJ foCompareTo, uint8_t cmd, uint8_t mode)
{
	...
	uint8_t  lfnFirstCheck = FALSE,foundSFN,foundLFN,fileFoundDotPosition = FALSE,fileCompareDotPosition;
        ...
        do      //test each possible boot sector (FAT32 can have backup boot sectors)
        {
            ...
            else   
            {
                ...
                do      //loop just to allow a break to jump out of this section of code
                {
                    ...
                    // Calculate the number of sectors in the root (will be 0 for FAT32)
                    RootDirSectors = ((dsk->maxroot * NUMBER_OF_uint8_tS_IN_DIR_ENTRY) + (BytesPerSec - 1)) / BytesPerSec;
                    // Calculate the number of data sectors on the card
...
int FSformat (char mode, long int serialNumber, char * volumeID)
{
    ...
    uint8_t *  dataBufferPointer = gDataBuffer;
#endif
...
#endif
    {
        ...
        else if (dsk->type == FAT12)
        {
            ...
            if (p == 0)
            {
                ...
                //if (WriteFAT (dsk, 0,0,TRUE))
                //    return ClusterFailValue;
                //TODO: commented this out to avoid recursion. 
...
#endif
{
    ...
    char Index, Index2;
		...
		uint8_t prevUtfModeFileName = utfModeFileName;
		char tempArray[514];
    	uint16_t prevHandle;
    	LFN_ENTRY *lfno;
    	FSFILE cwdTemp;
