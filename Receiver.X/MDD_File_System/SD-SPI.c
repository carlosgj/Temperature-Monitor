...
#if defined __C30__ || defined __C32__ || defined __STM32F10X__
...
#define WriteSPISlow        SPI2Transfer
...
uint8_t MDD_SDSPI_AsyncReadTasks(ASYNC_IO* info)
{
    ...
    static uint8_t SingleBlockRead;
...
static void PIC18_Optimized_SPI_Read_Packet(void){
    ...
    memset(ioInfo.pBuffer, 0xff, ioInfo.wNumBytes);
    SD_ExchangeBlock(ioInfo.pBuffer, ioInfo.wNumBytes);
...
static void PIC18_Optimized_SPI_Write_Packet(void){
    ...
    for(i=0; i<ioInfo.wNumBytes; i++){
        SPI2Transfer(ioInfo.pBuffer[i]);
    }
}  
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
	...
	else //if (CSDResponse[0] & 0xC0)	//Check CSD_STRUCTURE field for v1 struct device
	{
		...
		c_size_mult = (uint8_t)( ((uint16_t)((CSDResponse[9] & 0x03) << 1)) | ((uint16_t)((CSDResponse[10] & 0x80) >> 7)) );

        //Extract the BLOCK_LEN field from the response. It is a 4-bit number in bit position 83:80.
        block_len = CSDResponse[5] & 0x0F;

        block_len = (uint8_t)(1 << (block_len - 9)); //-9 because we report the size in sectors of 512 bytes each
