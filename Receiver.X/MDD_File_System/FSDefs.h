...
// Description: The NUMBER_OF_uint8_tS_IN_DIR_ENTRY macro represents the number of bytes in one directory entry.  It is used to calculate
//              the number of sectors in the root directory based on information in the boot sector.
#define NUMBER_OF_uint8_tS_IN_DIR_ENTRY    32
...
typedef struct {
    Suint16_t BootSec_JumpCmd;          // Jump Command
...
typedef struct {
    Suint16_t BootSec_jmpBoot;          // Jump Command
    uint8_t  BootSec_OEMName[8];       // OEM name
...
typedef struct
{
    ...
    Suint16_t     PTE_FrstPartSect;       // The cylinder-head-sector address of the first sector of the partition
    uint8_t      PTE_FSDesc;             // The file system descriptor
    Suint16_t     PTE_LstPartSect;        // The cylinder-head-sector address of the last sector of the partition
