...
uint8_t days_in_month(uint16_t year, uint8_t month){
    ...
    if(month < 1 || month > 12){
        printf("Assert failed in days_in_month: month not in range 1-12\n");
        return 0;
    }
...
uint16_t days_before_month(uint16_t year, uint8_t month){
    ...
    if(month < 1 || month > 12){
        printf("Assert failed in days_before_month: month not in range 1-12\n");
        return 0;
    }
...
uint32_t ymd2ord(struct date_t *ymd){
    ...
    uint8_t dim = days_in_month(ymd->year, ymd->month);
    //assert 1 <= day <= dim, ('day must be in 1..%d' % dim)
    if(ymd->dom < 1 || ymd->dom > dim){
        printf("Assert failed in ymd2ord: day not in range 1-%d\n", dim);
        return 0;
    }
    return days_before_year(ymd->year) + days_before_month(ymd->year, ymd->month) + ymd->dom;
}
...
void ord2ymd(uint32_t n, struct date_t *ymd){
    ...
    uint16_t n400 = n / DI400Y;
    n = n % DI400Y;
    
    ymd->year = n400 * 400 + 1;   // ..., -399, 1, 401, ...
    ...
    uint16_t n100 = (uint16_t)(n / DI100Y);
    n = n % DI100Y;

    // Now compute how many 4-year cycles precede it.
    //n4, n = divmod(n, _DI4Y)
    uint16_t n4 = (uint16_t)(n / DI4Y);
    n = n % DI4Y;

    // And now how many single years.  Again n1 can be 4, and again meaning
    // that the desired day is December 31 at the end of the 4-year cycle.
    //n1, n = divmod(n, 365)
    uint16_t n1 = (uint16_t)(n / 365);
    n = n % 365;

    ymd->year += n100 * 100 + n4 * 4 + n1;
    if(n1 == 4 || n100 == 4){
        if (n != 0){
            printf("Assert failed in ord2ymd: n != 0\n");
            return;
        }
        //return year-1, 12, 31
        ymd->year--;
    ...
    uint8_t leapyear = (n1 == 3 && (n4 != 24 || n100 == 3));
    //assert leapyear == _is_leap(year)
    if (leapyear != is_leap(ymd->year)){
        printf("Assert failed in ord2ymd: leapyear discrepancy\n");
        return;
    }
    
    ymd->month = (uint8_t)((n + 50) >> 5);
    uint16_t preceding = DAYS_BEFORE_MONTH[ymd->month] + (uint8_t)(ymd->month > 2 && leapyear);
    if(preceding > n){  // estimate is too large
        ymd->month--;
        preceding -= DAYS_IN_MONTH[ymd->month] + (uint8_t)(ymd->month == 2 && leapyear);
    }
    n -= preceding;
    
    //assert 0 <= n < _days_in_month(year, month)
    if( n >= days_in_month(ymd->year, ymd->month)){
        printf("Assert failed in ord2ymd: days_in_month incorrect\n");
        return;
    }
